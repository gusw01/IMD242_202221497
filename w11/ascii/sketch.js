...
function draw() {
  ...
  capture.loadPixels(); //업데이트 되는 정보 반영
  for (let idx = 0; idx < capture.pixels.length / 4; idx++) {
    ...
    gradientIdx = floor(gradientIdx); //정수화

    let asciiText = gradientText.charAt(gradientIdx);

    let coloum = idx % captureSize[0];
    let row = floor(idx / captureSize[0]); // idx / captureSize[0];을 정수화

    let x = coloum * tileWidth + tileWidth * 0.5;
